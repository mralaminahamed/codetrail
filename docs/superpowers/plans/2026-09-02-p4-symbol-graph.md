...
> of what was true when it was written. One premise has been closed and one count was wrong on the
> day:
>
> - **"The indexer has no `/metrics` endpoint."** True in P4. **P8's Task 2 closed it**: the indexer
>   serves `/health`, `/ready` and `/metrics` on `PROBE_PORT` (9090), so this phase's instruments
>   are reachable.
> - **"This phase's four instruments."** P4 added **three**, not four — `codetrail_graph_edges_total`,
>   `codetrail_typecheck_total` and `codetrail_typecheck_seconds` are the only `metrics.*` calls the
>   phase introduced. This was wrong when it was written rather than stale, and P8's own plan caught
>   it in review and recorded the arithmetic; P6 and P7 had already inherited "four" from these two
>   sentences by then, which is why it is corrected here at the source.
...
**Not in P4, deliberately:** no `imports` or `references` edges (Open question 7). No LLM tool loop — §8's `definition_of` and `callers_of` are P7's tools and this phase ships the endpoints beneath them. No console (P5). No eval harness (P6); nothing in this phase touches the two-arm corpus design. No incremental re-index (P7): a new commit is a new repo id and a full graph. No second language (§7). No `/metrics` on the indexer, so this phase's four instruments join P3's three with nowhere to be scraped from. *(**three**, not four — corrected 2026-09-06; see the note at the head of this file.)*
...
- **§11's counters still have nowhere to live in the indexer.** P3 recorded this; P4 adds four more instruments to the same gap. *(**three**, not four — corrected 2026-09-06; see the note at the head of this file.)* The indexer has no HTTP server, so job outcomes, admission rejections, evictions and now type-check outcomes are written and unscrapable. Giving the indexer a probe server is P0-shaped work and belongs in its own task rather than being smuggled in here.
